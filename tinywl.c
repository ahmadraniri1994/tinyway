...
static const char *
get_title(struct tinywl_view *view)
{
	const char *res;

	res = view->xdg_toplevel->title;
...
maximize(struct tinywl_server *server)
{
	...
	unsigned int tobt = top_or_bottom;
	const char *app_id;
	...
	struct wlr_output *output = getMonitorViewIsOn(view);
	...
	view->x = gapsize;
	view->y = gapsize + (((eexz * exczone) + (gapsize * eexz)) - ((tobt * exczone) + (gapsize * tobt)) * eexz);
	view->h = output->height - ( 2 * gapsize ) - ((eexz * exczone) + (gapsize * eexz));
	view->w = output->width - ( 2 * gapsize );
...
send_lower(struct tinywl_server *server)
{
	...
	app_id = get_app_id(view);
	if (strcmp(app_id, "lxqt-panel") == 0 || strcmp(app_id, "pcmanfm-qt") == 0)
		return;
